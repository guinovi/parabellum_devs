...
- [ ] [cite_start]**Modelo - Productos/Insumos:** Crear el CRUD básico para los productos que produce la planta[cite: 82].
- [ ] [cite_start]**Modelo - Pedidos (Core):** Crear el CRUD de pedidos asegurando la relación entre el actor (sucursal/franquicia) y los productos[cite: 81].
...
- [ ] [cite_start]**Interacción entre Módulos:** Validar que al crear un pedido, la sucursal/franquicia y los productos referenciados existan en el sistema[cite: 14, 15].
- [ ] [cite_start]**Manejo de Errores y Validaciones:** Implementar validaciones de datos obligatorios y retorno de códigos de estado HTTP adecuados[cite: 38, 39, 85].
...
- [ ] [cite_start]**Layout Base:** Crear el archivo `layout.pug` con la estructura HTML, header, footer y barra de navegación[cite: 157].
- [ ] [cite_start]**Vistas del Portal de Franquiciados:** Desarrollar las vistas para el "portal de pedidos" que exige el caso, permitiendo a los franquiciados realizar solicitudes estructuradas[cite: 83].
- [ ] [cite_start]**Vistas CRUD:** Implementar vistas para listar (ej. `index.pug`), ver detalle (ej. `detail.pug`), y crear/editar (ej. `nuevo.pug`) al menos para los pedidos y productos[cite: 155, 159, 160].

### Fase 4: Documentación
- [ ] [cite_start]**Nomenclatura del Documento:** Nombrar el archivo de documentación y la carpeta de Google Drive exactamente con el formato: `DSWB_2#_#################_1C26`[cite: 108].
...
- [ ] [cite_start]**Intro y Funcionalidades:** Redactar la introducción y los objetivos generales/funcionalidades del Caso 4[cite: 112, 116].
- [ ] [cite_start]**Módulos y Persistencia:** Describir los módulos desarrollados y explicar la persistencia de datos en JSON[cite: 117, 118].
- [ ] [cite_start]**Pruebas (Capturas):** Adjuntar capturas de pantalla de las peticiones HTTP probadas con Thunder Client (mostrando métodos GET/POST, URL y Status Codes)[cite: 154].
